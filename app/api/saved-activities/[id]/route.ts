...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await db.collection('savedActivities').findOne({ _id: new ObjectId(id) });
    if (!existing) return NextResponse.json({ message: 'Saved activity not found' }, { status: 404 });
